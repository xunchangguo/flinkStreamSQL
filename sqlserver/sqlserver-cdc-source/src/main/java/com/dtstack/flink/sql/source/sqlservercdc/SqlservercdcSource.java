package com.dtstack.flink.sql.source.sqlservercdc;

import com.alibaba.ververica.cdc.connectors.sqlserver.SqlServerSource;
import com.alibaba.ververica.cdc.debezium.DebeziumSourceFunction;
import com.dtstack.flink.sql.source.IStreamSourceGener;
import com.dtstack.flink.sql.source.sqlservercdc.table.RowDebeziumDeserializeSchema;
import com.dtstack.flink.sql.source.sqlservercdc.table.SqlserverCdcSourceTableInfo;
...
public class SqlservercdcSource implements IStreamSourceGener<Table> {
    ...
    public Table genStreamSource(AbstractSourceTableInfo sourceTableInfo, StreamExecutionEnvironment env, StreamTableEnvironment tableEnv) {
        SqlserverCdcSourceTableInfo sqlserverCdcSourceTableInfo = (SqlserverCdcSourceTableInfo) sourceTableInfo;
        ...
        TypeInformation[] types = new TypeInformation[sqlserverCdcSourceTableInfo.getFields().length];
        for (int i = 0; i < sqlserverCdcSourceTableInfo.getFieldClasses().length; i++) {
            types[i] = TypeInformation.of(sqlserverCdcSourceTableInfo.getFieldClasses()[i]);
        }
        TypeInformation typeInformation = new RowTypeInfo(types, sqlserverCdcSourceTableInfo.getFields());
        RowDebeziumDeserializeSchema deserializer = new RowDebeziumDeserializeSchema(
                rowType,
                typeInformation,
                ((rowData, rowKind) -> {}),
                ZoneId.of(sqlserverCdcSourceTableInfo.getServerTimeZone()));

        DebeziumSourceFunction<Row> cdcSource = SqlServerSource.<Row>builder()
                .hostname(sqlserverCdcSourceTableInfo.getHostname())
                .port(sqlserverCdcSourceTableInfo.getPort())
                .username(sqlserverCdcSourceTableInfo.getUsername())
                .password(sqlserverCdcSourceTableInfo.getPassword())
                .database(sqlserverCdcSourceTableInfo.getDatabaseName())
                .tableList(sqlserverCdcSourceTableInfo.getTableName())
                .serverTimeZone(sqlserverCdcSourceTableInfo.getServerTimeZone())
                .debeziumProperties(sqlserverCdcSourceTableInfo.getProperties())
                .deserializer(deserializer)
                .build();
        DataStreamSource dataStreamSource = env.addSource(cdcSource, sourceTableInfo.getName(), typeInformation);

        String fields = StringUtils.join(sqlserverCdcSourceTableInfo.getFields(), ",");
